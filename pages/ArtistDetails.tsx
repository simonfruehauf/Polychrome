...
const ArtistDetailsPage: React.FC = () => {
    ...
    if (!artistData) return <div className="text-neutral-500 p-8">Artist not found.</div>;

    const playAllTopTracks = () => {
        if(artistData.tracks.length > 0) {
            playContext(artistData.tracks, 0, artistData.name);
        }
    };
    ...
    return (
        <div>
            ...
            <div className="flex flex-col md:flex-row items-center md:items-end gap-6 mb-10 pb-6 border-b border-neutral-800">
                <img 
                    src={api.getArtistPictureUrl(artistData.picture, '750')} 
                    alt={artistData.name} 
                    className="w-48 h-48 md:w-64 md:h-64 rounded-full shadow-2xl object-cover"
                />
                <div className="flex-1 text-center md:text-left">
                    <h1 className="text-4xl md:text-6xl font-bold mb-4">{artistData.name}</h1>
                ...
                <div className="grid grid-cols-1 gap-1">
                    {artistData.tracks.map((track, index) => (
                        <div 
                            ...
                            onClick={() => playContext(artistData.tracks, index, artistData.name)}
                        >
